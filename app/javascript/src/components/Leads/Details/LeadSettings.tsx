...
const LeadSettings = ({ leadDetails, setLeadDetails, setShowLeadSetting }) => {
  ...
  const [stageCodeList, setStageCodeList] = useState<any>(null);
    ...
    const getAllowedUsers = async () => {
      ...
      leadItemsApi.get()
        .then((data) => {
          ...
          setStageCodeList(data.data.stage_codes);
...
qualityCodeList.map(e => <option value={e.id} key={e.id} selected={e.id === leadDetails.quality_code}>{e.name}</option>)}
              ...
              <div className="mb-6">
                <label className="block mb-2 text-sm text-gray-600">Stage</label>
                <select
                  defaultValue={leadDetails.stage_code}
                  className="w-full px-3 py-2 bg-miru-gray-100 border border-gray-300 rounded-md  focus:outline-none focus:ring focus:ring-indigo-100 focus:border-indigo-300"
                  name="stage_code" onChange={(e) => changeStateCode(e.target.value)}>
                  <option value=''>Select Stage</option>
                  {stageCodeList &&
stageCodeList.map(e => <option value={e.id} key={e.id} selected={e.id === leadDetails.stage_code}>{e.name}</option>)}
                </select>
                <div className="flex justify-between items-center pt-1 text-red-700">
                  {errors.stage_code && touched.stage_code &&
              <p className="text-xs">{`${errors.stage_code}`}</p>
                  }
                </div>
              </div>
