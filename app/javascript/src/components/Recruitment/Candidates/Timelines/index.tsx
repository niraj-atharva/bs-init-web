...
const Timelines = ({ candidateDetails, isDesktop }) => {
  ...
  const fetchLeadTimelines = () => {
    if (candidateDetails && candidateDetails.id) {
      leadTimelines.index(candidateDetails.id, queryParams())
        .then((res) => {
        ...
        <div className="my-6">
          <div className="flex flex-col w-full">
            {timelineData && timelineData.map((timeline) => (
              <div key={`${timeline.id}-timeline-section`} className="flex w-full h-full bg-white justify-beetween dark:bg-gray-800">
                <div className="flex w-full p-4 antialiased text-black bg-white dark:bg-gray-800 dark:text-gray-200">
                  {timeline.kind === 0 ?
                    <img className="w-8 h-8 mt-1 mr-2 rounded-lg " src={systemMessageIcon}/>
                    :
                    <img className="w-8 h-8 mt-1 mr-2 rounded-full " src={`${timeline.action_created_by && timeline.action_created_by.avatar_url ? timeline.action_created_by.avatar_url : profileDefaultAvatar}`}/>
                  }
